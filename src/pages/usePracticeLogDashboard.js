 ...
 * SOM â React hook for Practice Log page data
 *
...
// âââ Dev detection ââââââââââââââââââââââââââââââ
const IS_DEV = import.meta.env.DEV || import.meta.env.MODE === "development";

/**
 * @param {string} studentId  â Airtable record ID for the student
 * @param {object} opts       â { devMode: bool } force dev fallback
 */
export default function usePracticeLogDashboard(studentId, opts = {}) {
  ...
  // âââ Initial hydration ââââââââââââââââââââââââ
  ...
  useEffect(() => {
    ...
    (async () => {
      if ((devMode && !studentId.startsWith("rec")) || studentId === "demo") {
      ...
      try {
        ...
        // Fall back to demo data on API failure (backend endpoints not yet built)
  ...
  // âââ Calendar month change ââââ
  ...
  const createSession = useCallback(
    async (sessionData) => {
      const optimisticSession = {
        ...
        acc: "â",
        feel: { hard: "Hard", ok: "OK", great: "Great" }[sessionData.self_rating] || "â",
        d: 0, p: 0, m: 0, amb: "",
  ...
  // âââ Demo data loader (dev only) âââââ

  function loadDemoData() {
    setStudent({ id: "demo", name: "Demo Student", instrument: "Piano", grade: "6th Grade", school: "Westside Music", level: 4 });
    setPeriodData(DEMO_PERIOD_DATA);
    setSessions(DEMO_SESSIONS);
    setCalendarData({});
  }

  // âââ Return âââââââââââââââââââââââââââââââ

  return {
    status, error, retry, isDevMode: devMode,
    student, periodData, sessions, setSessions, calendarData,
    calMonth, calYear, navigateCalendar,
    createSession, loadMoreSessions, sessionsLoading, sessionTotal,
  };
}


// DEMO DATA (dev mode only)
const DEMO_PERIOD_DATA = {
  week: {
    ...
    dpm: { d: 78, p: 65, m: 82 },
    pieces: [
      { name: "C Major Scale", meta: "8 sessions Â· 92% accuracy", pct: 92 },
      { name: "Hanon Exercise No. 1", meta: "5 sessions Â· 74% accuracy", pct: 74 },
      { name: "Scale Recognition", meta: "3 sessions Â· 61% accuracy", pct: 61 },
    ],
    insight: "Thursday was your strongest session this week. Drive and Motivation are both strong â let's work on Passion by exploring a piece you personally love.",
  ...
  month: {
    ...
    dpm: { d: 81, p: 70, m: 85 },
    pieces: [
      { name: "C Major Scale", meta: "22 sessions Â· 94% accuracy", pct: 94 },
      { name: "Hanon Exercise No. 1", meta: "16 sessions Â· 79% accuracy", pct: 79 },
      { name: "Scale Recognition", meta: "10 sessions Â· 68% accuracy", pct: 68 },
    ],
    insight: "Week 3 was your best month yet â 89 minutes! You are most consistent mid-week.",
  ...
  quarter: {
    ...
    dpm: { d: 83, p: 72, m: 87 },
    pieces: [
      { name: "C Major Scale", meta: "55 sessions Â· 95% accuracy", pct: 95 },
      { name: "Hanon Exercise No. 1", meta: "38 sessions Â· 83% accuracy", pct: 83 },
      { name: "Scale Recognition", meta: "28 sessions Â· 74% accuracy", pct: 74 },
    ],
    insight: "You've improved 73% in practice minutes from October to January.",
  ...
  semester: {
    ...
    dpm: { d: 79, p: 68, m: 83 },
    pieces: [
      { name: "C Major Scale", meta: "80 sessions Â· 95% accuracy", pct: 95 },
      { name: "Hanon Exercise No. 1", meta: "52 sessions Â· 86% accuracy", pct: 86 },
      { name: "Scale Recognition", meta: "35 sessions Â· 77% accuracy", pct: 77 },
    ],
    insight: "September to January shows 160% growth. You're most productive on Tuesdays and Thursdays.",
    personalBests: ["42", "8", "312"],
  },
};

const DEMO_SESSIONS = [
  { id: 0, title: "C Major â Hands Together", date: "Today", type: "Homework", typeKey: "hw", dur: 22, acc: "87%", feel: "OK", d: 82, p: 71, m: 88, amb: "Your left hand timing improved this session.", source: "School" },
  { id: 1, title: "Hanon Exercise No. 1", date: "Yesterday", type: "Sheet Music", typeKey: "sm", dur: 18, acc: "74%", feel: "OK", d: 75, p: 68, m: 79, amb: "Solid session. Try isolating fingers 3 and 4.", source: "School" },
  { id: 2, title: "Find the Note â Level 3", date: "Tue", type: "Games", typeKey: "gm", dur: 13, acc: "91%", feel: "Great", d: 88, p: 90, m: 85, amb: "91% accuracy on Level 3 â personal best!", source: "Standalone" },
  { id: 3, title: "Scale Recognition Practice", date: "Mon", type: "Homework", typeKey: "hw", dur: 11, acc: "62%", feel: "Hard", d: 65, p: 55, m: 70, amb: "Short but focused â that counts.", source: "School" },
  { id: 4, title: "Live Practice Session", date: "Mon", type: "Live Practice", typeKey: "lp", dur: 12, acc: "79%", feel: "OK", d: 78, p: 72, m: 80, amb: "Good energy in this session.", source: "Standalone" },
];
